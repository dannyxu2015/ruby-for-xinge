...
module Xinge
  class Ios < Base
    ...
    def push_to_single_device(token, title, content, params={}, custom_content={}, send_time='')
      p = params.merge(environment: ENV_VAL)
      p.merge!(send_time: send_time) if send_time.present?
      self.push_single_device(token, 0, build_simple_message(title, content, custom_content), p)
    end
